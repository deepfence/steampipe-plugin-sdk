...
import (
	...
	postgres_store "github.com/deepfence/gocache/store/postgresqlcache/v4"
...
func (c *QueryCache) createPostgresqlCacheStore() (*postgres_store.PostgresqlStore, error) {
	return postgres_store.NewPostgresqlStore(fmt.Sprintf("postgres://%s:%s@%s:%s/%s?sslmode=%s&pool_max_conns=%d&pool_min_conns=%d&pool_health_check_period=%s",
...
func (c *QueryCache) ClearForConnection(ctx context.Context, connectionName string) error {
	return c.cache.Invalidate(ctx, libstore.WithInvalidateTags([]string{connectionName}))
}
